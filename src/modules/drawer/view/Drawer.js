...
function OtherStack() {
  return (
    <Stack.Navigator headerMode={false} initialRouteName="Other">
      ...
      <Stack.Screen
        component={OtherDatabaseReport}
        name="OtherDatabaseReport"
      />
    </Stack.Navigator>
  );
}
...
function SwithRouter() {
  ...
  useEffect(async () => {
    await apiDatabaseInit(dispatch);
    await apiCreateCategory(dispatch);
    await apiCreateItem(dispatch);
    await apiCreateTrxHeader(dispatch);
    await apiCreateTrxDetail(dispatch);
    await apiCreateStore(dispatch);
  ...
  return (
    <Stack.Navigator headerMode={false} initialRouteName="App">
      {!loading ? (
        <Stack.Screen component={DrawerTab} name="App" />
      ) : (
        <Stack.Screen component={Splash} name="Loading" />
      )}
    </Stack.Navigator>
  );
}
